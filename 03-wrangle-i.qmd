...
```{r}
## Without |>
mutate(filter(select(df, x1txmtscor), x1txmtscor > 50), sqrt = sqrt(x1txmtscor))
```

###### With `|>`
```{r}
## With |>
df |> select(x1txmtscor) |> filter(x1txmtscor > 50) |> mutate(sqrt = sqrt(x1txmtscor))
```
...
temp <- mutate(temp, sqrt = sqrt(x1txmtscor))
temp
```
