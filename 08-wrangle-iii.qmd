...
You may have noticed already that when we load the tidyverse library with `library(tidyverse)`, the stringr library is already loaded.
The lubridate library, though part of the tidyverse, is not.
...
As higher education scholars, IPEDS data are a valuable resource that you may often turn to (I do).
...
So far, we've filtered data using dplyr's `filter()` verb.
...
[Regular expressions](https://en.wikipedia.org/wiki/Regular_expression) (aka *regex*) are strings that use a special syntax to create patterns that can be used to match other strings.
...
In general, I find it's a good idea to try a solution like the second one before a more complicated one like the first.
...
It is has to work both for `zip` values that include the additional hyphen and 4 digits as well as those that only have the primary 5 digits to begin with.
...
From then on out, it adds everything it sees the second subexpression: `"-"` `"1"` `"2"` `"3"` `"4"`.
...
> What bit of two bits of code above would you change so that you can store the last 4 digits without including the hyphen?
...
To be honest, I've spent more time cursing than thanking them.
That said, regular expressions are often *the only* way to perform a data wrangling task on unstructured string data.
...
-   Now, remember `orders` can take more than one value, so maybe we could try another date format that can pick up some of these dates
    ...
    -   Looking through the [lubridate reference page](https://lubridate.tidyverse.org/reference/index.html), I can see that `"my"` (month year) is a format it will take as well, so let's try that
...
Use the IPEDS data sets, `hd2007.csv`and `ic2007mission.csv`, to answer the questions below.
You may need to look up and download the data dictionaries for each file, which you can find on [this page](https://nces.ed.gov/ipeds/datacenter/DataFiles.aspx).
[You can also use the lesson on getting higher education data for help](02-set-data.qmd)

For each question, show your data work and then answer the question in a short (1-2 sentence(s)) comment.

## Questions

**NB** To answer the questions, you will need to join the two IPEDS data sets using the common `unitid` key.
Note that column names in `hd2007.csv` are uppercase (`UNITID`) while those in `ic2007mission.csv` are lowercase (`unitid`).
There are a few ways to join when the keys don't exactly match.
One is to set all column names to the same case.
If you want to use `left_join()` starting with `hd2007.csv`, you can first use the the dplyr verb `rename_all(tolower)` in your chain to lower all column names.
[See the help file for `left_join()`](https://dplyr.tidyverse.org/reference/join.html) for other ways to join `by` different variable names.
...
3)  Among those schools that give their mission statement:
    i)  How many repeat their institutional name in their mission statement?\
    ii) How many use the word *civic*?
    iii) Which top 3 states have the most schools with mission statements that use the word *future*?\
    iv) Which type of schools (public, private-non-profit, private-for-profit) are most likely to use the word *skill* in their mission statement?
4)  Among the schools that closed in 2007 or 2008 and give a date with at least a month and year:
    i)  Which has been closed for the longest time? How many months has it been from its close date to the beginning of this current month (1 February 2020)?\
    ii) How many days were there between the first school to close and the last?
...
[{{< fa code >}} R Solution Code](/solutions/data-wrangling-iii-solution.R)

```{r}
#| echo: true
#| eval: false
#| file: solutions/data-wrangling-iii-solution.R
```
:::
:::
