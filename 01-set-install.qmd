...
ii. For mac users, select either apple silicon or intel options
    depending on how new your mac is - If you need to check which kind
...
-   *Technically*, R is all you need to do all of our analyses. However,
    ...
    -   The easiest way to use programming languages is through a
        ...
        -   There are multiple "development environments" you can use
            ...
            and it is purpose built for the language (it also works with
            Python too)
...
For today, we are mostly going to explore R using the console, copying and pasting commands from the website. Fear not, however, as there is a `.R` script with the contents in your class folder that we will set up next week.
...
-   If we want to, we could actually put these commands directly into
    the console (try copying `1 + 1` into the console)
...
-   You may notice ~~I~~ (Ben, and therefore, now me) use two hashes
    ...
    -   If you want to take your comments to the next level `##'`
        RStudio also enables some fun color coding options with `@`,
        `[]`, `**`, and `:`
...
-   You may hear me use the phrases "base R" during class
    -   What I mean by this is the R that comes as you download it with
        no packages loaded (sometimes also called "vanilla R")
...
> package name: `install.packages("tidyverse")`
