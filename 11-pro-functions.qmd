...
1. Using a loop, do the following:
   i. Read in each of the individual school test files for Bend Gate
      and Niagara only. (**HINT** The vertical pipe operator, `|`,
      means **OR** in regular expression patterns.)  
   ii. Within each iteration of the loop, add a column to the data
   ...
   iii. Bind all the data sets together.
2. Read in `hsls_small.csv` and do the following:
   i. Using the user-written function `fix_missing()`, convert missing
      values in `x1ses` to `NA`.  
	 ii. Subset the full data frame to the first 50 rows and pull out
		 ...
		 test_scr <- df |> 
			...
			pull(x1txmtscor)
		 ```
	 iii. Using a `for()` loop, print out the index of the missing
	  values (when `test_scr` equals `-8`).
	 iv. Repeat the same code, but add an `else()` companion to the
         initial `if()` statement that prints the value if non-missing.
	 v. Add an `else if()` between the initial `if()` and final
   ...
   vi. Write your own function to compare two values and return the
