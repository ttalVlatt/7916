...
- For this section, we are going to see how tasks from [Data Wrangling I](03-wrangle-i.qmd) and [Data Wrangling II](04-wrangle-ii.qmd) could be performed using `SQL`, a common tool used to work with databases.
...
- To get started, we need to read in our data and set it up as a simulated database, just like we did above
...
- Much like before, we start with `SELECT`
...
- We are taking them from our simulated databases `dbplyr_002` which we want `GROUP BY`-ed year
  - Remember we can't read nested code top to bottom, the `GROUP BY` takes place as we pull the data out of `dbplyr_002` before we calculate the `AVG()`
...
- What we see here is a bit different, as it's manually
