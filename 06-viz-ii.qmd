...
- I've included a couple of new things here, but it should be mostly familiar
  - `scale_fill_viridis_d()` sets color-blind friendly palettes, which can be great when you have a categorical variable but
  ...
  - `begin` and `end` just allow you to trim the more extreme ends of the color palette off
        - This is useful in situations like this when you only have two cateogories and don't want them to look totally black and white
...
- Cool! But there's a few things I don't like...
...
- In the preview pane, it's super squished, and even if we zoom in it's not great...
...
- That's definitely better! If I'm getting picky, the boxplot is a little streched though, as it's trying to fill the length of two histograms
...
- Pretty good, but when can do even better...
  ...
  - Then inside `plot_layout()` specify `guides = "collect"` which will try and put all the legends in one place
    - If our legends matched exactly, it would only keep one, but as the boxplot lines are part of the legened, they technically cannot be collapsed
