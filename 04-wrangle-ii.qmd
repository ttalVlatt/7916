...
- In this second example, we'll again reshape from wide to long, but we'll have to wrangle our data a bit after the reshape to make it analysis ready.

- First, we'll read in a second file `all_schools_wide.csv`.
  - This file contains the same information as before, but in a _very_ wide format: each school has only one row and each test by year value gets its own column in the form `<test>_<year>`.
...
- This mostly worked to get our data long
  - But now we have this weird combined `test_year` column.
  - What we really want are two columns
    - One for the year
    - One for the test type.
      - We can fix this using [tidyr `separate()`](https://tidyr.tidyverse.org/reference/separate.html) function with the following arguments:
...
#### Quick exercise 
- Redo the last two steps in a single combined chain using pipes.
  - That is, start with `df` (which contains `all_schools_wide.csv`)
  - Reshape it long
  - Fix so that you end up with four columns
...
1. In one chained pipe (I don't need to see intermediate steps);
  ...
  iv. Finally, return the mean of these differences by region.
